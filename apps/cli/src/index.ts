...
type WorkspaceRunNextStatus = "planned" | "executed" | "blocked";

interface WorkspaceRunNextPlan {
  ...
  reviewId: string;
  item?: Pick<WorkspaceReviewItem, "itemId" | "kind" | "priority" | "title" | "summary" | "command" | "routeId" | "obligationId" | "obligationKind" | "claimId" | "sessionId">;
  execution: {
    status: WorkspaceRunNextStatus;
    kind: string;
    summary: string;
...
async function createWorkspaceRunNextPlan(input: {
  rootPath: string;
...
function workspaceRunNextItemSummary(
  item: WorkspaceReviewItem
): WorkspaceRunNextPlan["item"] {
  return {
    itemId: item.itemId,
    kind: item.kind,
...
async function executeWorkspaceRunNextItem(
  ...
  const workspace = String(options.workspace ?? rootPath);
