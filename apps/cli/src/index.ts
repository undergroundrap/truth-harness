...
hum
  ...
  .action(async (inputs: string[], options: { kind: string; json?: boolean; allowUnknownSchemaVersion?: boolean }) => {
    const result = await validateHumContractInputs({
      inputs: inputs.length > 0 ? inputs : ["-"],
      kind: parseHumValidateKind(options.kind),
      allowUnknownSchemaVersion: options.allowUnknownSchemaVersion === true
    });

    if (options.json) {
      printJson(result);
    } else {
      printHumValidateSummary(result);
    }
...
workspace
  ...
  .option("--summary-json", "Print compact release audit status JSON without embedded snapshots")
  ...
  .action(
    async (
      ...
      if (options.summaryJson) {
        printJson(createReleaseAuditCliSummary(audit));
...
workspace
  ...
  .option("--summary-json", "Print compact credibility-pack status JSON without embedded snapshots")
  .option("--max-routes <count>", "Maximum route summaries to inspect; use 0 to skip routes", parseNonNegativeInteger)
  .option("--max-claims <count>", "Maximum claim records to inspect; use 0 to skip claims", parseNonNegativeInteger)
  .option("--max-sessions <count>", "Maximum research sessions to inspect; use 0 to skip sessions", parseNonNegativeInteger)
  .option("--max-reports <count>", "Maximum saved report drafts to inspect; use 0 to skip reports", parseNonNegativeInteger)
  ...
  .action(
    async (
      ...
      if (options.summaryJson) {
        printJson(createCredibilityPackCliSummary(pack, writeResult));
      } else if (options.json) {
...
function printReleaseAudit(audit: ReleaseAudit): void {
  ...
  console.log(`  ${audit.commands.dockerMathlibTemplate}`);
...
function printHumValidateSummary(result: HumValidateReport): void {
  console.log(`Hum validate: ${result.status}`);
  console.log(`Inputs: ${result.summary.valid}/${result.summary.total} valid, ${result.summary.invalid} invalid, ${result.summary.tool_errors} tool error(s)`);
  for (const input of result.inputs) {
    ...
    for (const issue of input.issues) {
      console.log(`  ${issue.path}: ${issue.message} [${issue.rule}]`);
    }
  }
