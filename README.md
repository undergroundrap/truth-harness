...
The engine manifest also exposes the agent contract for future high-performance work: determinism class, primitive semantics, replay requirements, drift risks, and stable JSON diagnostics for each capability. That is the bridge from today's exact math receipts to later Rust kernels, rigorous numerics, simulation records, and differential-fuzzing oracles without pretending stochastic output or AI-generated code is truth by itself.
