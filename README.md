...
Release audit commands compose the readiness checks into one local professor/public-review gate. `npm run audit:release` reports workspace status, validation, catalog freshness, engine evidence, saved engine-run posture, review queue blockers, code-run sandbox status, and UI-launch warnings without starting Docker or executing arbitrary code. `npm run audit:release:gate` is the strict failing gate: it requires Maxima, Z3, cvc5, Lean, and SageMath evidence gates, a saved strict engine-run record, and a measured code-run sandbox. If that strict gate is blocked, the result is intentional: the workbench is refusing to pretend it is release-ready.
