...
describe("benchmark runner", () => {
  ...
  it("keeps the engine math seed suite replayable", () => {
    ...
    expect(run.failed).toBe(0);
    expect(run.trustAccuracy).toBe(1);
    expect(trustCounts).toMatchObject({ "exact-computed": 8, refuted: 4 });
    expect(backendIds).toEqual(new Set(["local-aabb2-overlap", "local-segment2-intersection", "local-ray2-aabb-intersection", "local-point-in-triangle2"]));
