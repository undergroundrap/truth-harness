...
function renderSmtLib(args: {
  ...
  const lines = [
    "; Generated by Theorem Workbench from explicit local structured constraints.",
...
async function requireLocalWorkspace(
  ...
  if (!status.exists || !status.manifest) {
    throw new Error("No Theorem workspace found. Run `theorem workspace init` before solving SMT problems.");
  }
