...
Credibility-pack, reviewer-bundle, bundle-verification, and report-draft JSON are validated by schemas during workspace validation, so reviewer packets are checked as first-class local artifacts instead of loose notes.
