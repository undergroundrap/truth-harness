...
- A one-command Docker professor evidence route: `npm run docker:professor` builds the pinned Lean proof image, then writes Maxima/Z3/Lean engine evidence, adversarial benchmark evidence, and a credibility pack from inside the no-network compose service.
