...
Headless reviewers and agents can inspect the same saved drafts with `truth-harness workspace reports .` and `truth-harness workspace report <report_...> . --markdown`; both paths use the same core Markdown hash verification as the web Report tab.
