...
The web Report tab renders the same action plan, exposes copy buttons for those commands, shows the latest reviewer-bundle checklist, displays the archive SHA-256, and can download the latest bundle README, manifest, credibility-pack Markdown, `.tar.gz` archive, or `.sha256` sidecar through local-only read endpoints. It can also ask `/api/workspace-run-next?source=credibility-actions` for the next browser-safe reviewer plan. That web path is dry-run only; it shows and copies the same local command that CLI/MCP can execute through the shared gated planner. The Report tab can also save the dry-run plan as JSON/Markdown under `.truth-harness/findings/`, creating an auditable intent packet before any agent or human runs the command.
