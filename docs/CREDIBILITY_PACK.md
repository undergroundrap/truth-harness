...
Workspace-review, run-next, credibility-pack, reviewer-bundle, bundle-verification, and report-draft JSON are validated by schemas during workspace validation, so reviewer packets and agent handoffs are checked as first-class local artifacts instead of loose notes. Saved report drafts also re-check their Markdown sidecar SHA-256 and byte length, so the human-facing review text cannot silently drift away from the JSON receipt.
