# Public Answers

This folder stores human-readable answer packets for public math prompts that Truth Harness can solve, refute, or honestly bound with replayable evidence.

Each packet should follow [Public Answer Workflow](../PUBLIC_ANSWER_WORKFLOW.md) and cite:

- the source problem,
- the normalized claim,
- the trust label,
- receipts or benchmark suites,
- replay commands,
- limitations and review status.

## Packets

| Date | Packet | Status | Trust label | Source |
| --- | --- | --- | --- | --- |
| 2026-07-03 | [Pythagorean trig identity](2026-07-03-pythagorean-trig-identity.md) | self-reviewed, not posted | `cross-checked`; near-miss `refuted` | Wikipedia |

Public answer packets are not proof by themselves. They are readable entry points into the local evidence ledger.
