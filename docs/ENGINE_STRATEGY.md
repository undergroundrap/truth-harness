...
Reviewer gates should make that independence explicit. Daily local checks can use the faster Docker-core route of Maxima plus Z3 and cvc5, while `--require-all-engines` is the stricter professor/auditor mode: Maxima, Z3, cvc5, Lean, and SageMath must each earn their own scoped fixture evidence before the engine packet is marked ready under that standard. `npm run docker:all-engines` is the heavy no-runtime-network container route for that gate, and `npm run docker:all-engines:write` saves the resulting strict engine-run record for credibility-pack and release-audit citation. Passing this gate proves engine evidence for representative fixtures only; individual claims still need their own receipts, routes, and satisfied obligations.
...
When a workspace has saved strict Docker reviewer evidence, `engines plan`, `workspace run-next`, and MCP run-next include that memory as a routing hint. This lets agents see that the reviewer image has previously covered Maxima, SageMath, Lean, Z3, and cvc5 even when the host machine has none of those commands installed. The boundary is deliberately conservative: saved reviewer evidence does not make a missing host engine runnable, does not close a current claim, and does not mint `proved`, `smt-checked`, or `cross-checked` for a new statement. It only points the agent toward the right Docker reviewer gate or the concrete proof/SMT/CAS artifact still required for the current claim.

`workspace run-next` now embeds the same routing contract when the selected work item carries a scoped claim or verifier command. That means an autonomous agent reopening a saved handoff does not need to guess whether a blocker is exact arithmetic, symbolic CAS, SMT, source-grounded, simulation-grade, or formal-proof work: the run-next packet names the first durable command, the relevant verifier stack, the evidence each engine must produce, and the stronger engines that remain blocked. Once a proof validation gate already has weak or unverified attached evidence, run-next treats that as progress but not closure: the proof gate drops behind concrete route obligations, or it recommends an explicit stronger-engine boundary such as `npm run docker:engines`, `npm run docker:sage`, or `docker compose run --rm lean-proof` instead of rerunning the same weak verifier. If a matching ready verifier route exists for a linked validation plan, run-next proposes `truth-harness validation attach ... --evidence route:<id>` before claim recording; only after the gate is satisfied does it propose a domain-aware `claim add` command for the narrow route-backed claim. This is still planning, not evidence; concrete receipts, proof-check records, SMT records, CAS records, source citations, or validation attachments remain the only artifacts that can move trust labels.
