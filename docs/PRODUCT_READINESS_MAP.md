...
| Docker engine path | Green | Docker image includes SymPy, Maxima, Z3, cvc5, and a pinned Lean proof target; `engines verify --require-maxima --require-z3 --require-cvc5` reports earned `cross-checked` and independent `smt-checked` gates in the no-network compose service. Engine verification now also emits named external-engine levels, including the strict all-engine reviewer gate when Maxima, Z3, cvc5, Lean, and SageMath each mint scoped evidence. `npm run docker:professor` writes Maxima/Z3/cvc5/Lean engine evidence, adversarial benchmark evidence, math credibility ladder evidence, professor math challenge evidence, exact/symbolic/SMT hard-math closure evidence, a credibility pack, and a hash-verified portable reviewer bundle in one no-network reviewer rehearsal. `npm run docker:all-engines` is the heavier strict gate that combines SageMath plus pinned Lean so Maxima, Z3, cvc5, Lean, and SageMath each earn scoped fixture evidence in one no-runtime-network image. `npm run docker:professor:all` writes that same strict all-engine standard plus closure reports into the generated professor credibility pack and portable reviewer bundle, then release audit can cite the saved `cver_...` bundle verification only when copied files, manifest digest, and source drift are clean. `npm run docker:sandbox:write` writes a durable no-network sandbox measurement for release-audit citation. |
...
| Adversarial and closure evidence | Green/yellow | The 16-case `ai-failure-seed` suite can be saved as a benchmark-run record; `math-credibility-ladder` is now a 30-case native-safe hard-math readiness suite for exact equality/refutation, signed common-denominator lemmas, parity boundaries, dimensional powers and mistakes, interval bounds, and honest theorem/parser-boundary refusals; `professor-math-challenge` adds a 26-case native-safe reviewer exam for exact algebra slips, finite integer claims, units, interval boundaries, and honest frontier refusals; Docker hard-math closure reports now prove the seeded exact, symbolic CAS, and SMT blockers can be routed to scoped evidence and saved as reviewer artifacts; professor credibility packs/release audits block readiness when any required benchmark run or closure report is missing/failing; run-next can close benchmark actions through core APIs without shell execution while Docker closure actions remain explicit container-boundary commands; reviewer packets cite artifact paths, replay commands, receipt replay examples, trust accuracy, and closure ledger paths. Needs larger public false-claims corpus. |
...
4. `npm run docker:professor` completes, writes local engine/benchmark/closure/credibility-pack/reviewer-bundle artifacts, writes a saved `cver_...` reviewer-bundle verification with manifest digest `verified`, and the resulting credibility pack itself reports `ready-for-review`.
5. `npm run docker:all-engines` passes when the heavy SageMath plus Lean image is available; `npm run docker:all-engines:write` can save a strict run for audit citation; `npm run docker:professor:all` can package that strict standard into the professor reviewer bundle.
6. `npm run docker:demo` passes.
7. `npm run demo:math-ladder` and `npm run demo:professor-math` pass, and any failed case is treated as a hard-math credibility regression.
8. `npm run docker:hard-math-closure`, `npm run docker:symbolic-closure`, and `npm run docker:smt-closure` pass and write citable closure reports before professor/release readiness is considered green.
...
- Keep `packages/benchmarks/suites/math-credibility-ladder.json` and `packages/benchmarks/suites/professor-math-challenge.json` passing as native-safe hard-math reviewer floors before adding heavier Lean/Sage/mathlib cases.
...
2. **Browser layout regression:** `workspace release-audit` now composes workspace validation, catalog freshness, credibility-pack engine gates, saved engine-run posture, saved reviewer-bundle verification, saved adversarial benchmark posture, saved math-ladder posture, saved report draft integrity, review queue blockers, sandbox posture, and saved web UI review status into one local readiness report, and the Checks tab mirrors that report through the localhost `/api/release-audit` gate. The web app exposes `window.truthHarnessUiAudit()` and the read-only `http://127.0.0.1:4180/?uiAudit=1` path for browser automation to check every core surface for visible horizontal overflow, clipped content, branch-map overlap, and nested scroll traps. `workspace ui-review --layout-audit <json> --write` can now attach that layout-audit JSON to the durable `truth-harness.web-ui-review.v0` record, and warning/failure audit status becomes warning/failure review status. Next, add screenshot capture and pixel-diff baselines to the same review artifact.
3. **Workspace scale fixtures:** expand `workspace stress` beyond the first receipts/claims/routes harness into large mixed-artifact runs, performance budgets, release-audit latency checks, and web API latency checks.
4. **Claim search and dependency UX:** make tags, lineage, supersession, and linked claims obvious in UI and CLI.
5. **Professor credibility packets:** `workspace credibility-pack` now writes a local reviewer packet with validation status, embedded artifact hashes, engine evidence, saved adversarial benchmark evidence, saved math credibility ladder evidence, saved professor math challenge evidence, saved exact/symbolic/SMT hard-math closure evidence, open obligations, an actionable reviewer plan, and rerun commands. `workspace credibility-actions --json` exposes the same unresolved closure queue directly for agents and CI without executing anything. `workspace run-next --source credibility-actions` can now plan or execute one supported reviewer action through core APIs, including writable engine verification runs and writable benchmark runs, without shell execution; Docker closure items stay explicit manual/container commands. `workspace credibility-bundle` now copies the cited canonical artifacts into a portable directory and stamps the manifest with a digest over reviewer-critical metadata; `workspace verify-credibility-bundle` checks copied-file hashes, manifest metadata integrity, and source workspace drift separately. Next harden it with optional signing and compressed export.
...
10. **Adversarial benchmark suite:** the first 16-case seed suite, 30-case math credibility ladder, and 26-case professor math challenge are wired into professor/release readiness with named level summaries in saved benchmark records; expand both into a public false-claims and hallucination-catcher corpus before public launch.
