...
2. **Browser screenshot regression:** `workspace release-audit` now composes workspace validation, catalog freshness, credibility-pack engine gates, saved engine-run posture, saved adversarial benchmark posture, saved report draft integrity, review queue blockers, sandbox posture, and saved web UI review status into one local readiness report, and the Checks tab mirrors that report through the localhost `/api/release-audit` gate. `workspace ui-review --write` is the interim durable browser-review artifact. Next, add automated browser screenshot checks for clipping, overflow, focus states, and report readability.
