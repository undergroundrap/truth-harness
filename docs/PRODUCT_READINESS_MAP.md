...
2. **Browser screenshot regression:** `workspace release-audit` now composes workspace validation, catalog freshness, credibility-pack engine gates, saved engine-run posture, review queue blockers, sandbox posture, and UI-launch warnings into one local readiness report, and the Checks tab mirrors that report through the localhost `/api/release-audit` gate. Next, add automated browser screenshot checks for clipping, overflow, focus states, and report readability.
