...
8. The project can run a 100+ receipt local workspace without slow scans becoming painful.
9. Claim ledger search/tag/filter/dependency flows are usable from both CLI and UI.
10. At least one professor-style teaching workflow can be completed end to end: ask -> receipt -> trace -> notes -> report -> replay.
11. Agent workflows can work for at least one hour against local artifacts without losing provenance.
12. Security docs clearly explain what Docker protects, what it does not protect, and what code-run can and cannot attest.
...
2. **Browser screenshot regression:** `workspace release-audit` now composes workspace validation, catalog freshness, credibility-pack engine gates, saved engine-run posture, saved adversarial benchmark posture, saved report draft integrity, review queue blockers, sandbox posture, and UI-launch warnings into one local readiness report, and the Checks tab mirrors that report through the localhost `/api/release-audit` gate. Next, add automated browser screenshot checks for clipping, overflow, focus states, and report readability.
