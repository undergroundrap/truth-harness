...
| Docker engine path | Green | Docker image includes SymPy, Maxima, and Z3; `engines verify --require-maxima --require-z3` reports earned `cross-checked` and `smt-checked` gates in the no-network compose service. `npm run docker:professor` now writes Docker-core engine evidence, adversarial benchmark evidence, and a credibility pack in one no-network reviewer rehearsal. |
...
2. **Browser screenshot regression:** `workspace release-audit` now composes workspace validation, catalog freshness, credibility-pack engine gates, saved engine-run posture, saved adversarial benchmark posture, review queue blockers, sandbox posture, and UI-launch warnings into one local readiness report, and the Checks tab mirrors that report through the localhost `/api/release-audit` gate. Next, add automated browser screenshot checks for clipping, overflow, focus states, and report readability.
