...
| Docker engine path | Green | Docker image includes SymPy, Maxima, Z3, cvc5, and a pinned Lean proof target; `engines verify --require-maxima --require-z3 --require-cvc5` reports earned `cross-checked` and independent `smt-checked` gates in the no-network compose service. `npm run docker:professor` writes Maxima/Z3/cvc5/Lean engine evidence, adversarial benchmark evidence, math credibility ladder evidence, a credibility pack, and a hash-verified portable reviewer bundle in one no-network reviewer rehearsal. `npm run docker:all-engines` is the heavier strict gate that combines SageMath plus pinned Lean so Maxima, Z3, cvc5, Lean, and SageMath each earn scoped fixture evidence in one no-runtime-network image. `npm run docker:professor:all` writes that same strict all-engine standard into the generated professor credibility pack and portable reviewer bundle. `npm run docker:sandbox:write` writes a durable no-network sandbox measurement for release-audit citation. |
...
| Adversarial benchmarks | Green/yellow | The 16-case `ai-failure-seed` suite can be saved as a benchmark-run record; `math-credibility-ladder` is now a 30-case native-safe hard-math readiness suite for exact equality/refutation, signed common-denominator lemmas, parity boundaries, dimensional powers and mistakes, interval bounds, and honest theorem/parser-boundary refusals; professor credibility packs/release audits block readiness when either saved run is missing or failing; run-next can close both benchmark actions through core APIs without shell execution; reviewer packets cite artifact paths, replay commands, receipt replay examples, and trust accuracy. Needs larger public false-claims corpus. |
