...
| Adversarial benchmarks | Green/yellow | The 16-case `ai-failure-seed` suite can be saved as a benchmark-run record, and professor credibility packs/release audits now block readiness when that run is missing or failing. Needs larger public false-claims corpus. |
