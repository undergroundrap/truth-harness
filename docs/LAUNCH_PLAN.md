...
- Add local direct code execution records for scripts/tests/utilities agents claim to run. Status: core record, schema, default-deny executable allowlists, async process capture, timeout/output caps, per-workspace concurrency limiting, unsandboxed `networkAccess: unknown` privacy records, MCP opt-in gate, workspace validation, CLI, MCP, and regression coverage done; richer sandbox isolation still pending.
