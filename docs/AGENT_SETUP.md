...
- `truth_harness_workspace_credibility_actions` computes the same reviewer posture as a credibility pack but returns only the unresolved action queue. It is read-only, local-only, and should be the first MCP call when an agent needs to know which validation, engine, or workspace-review blocker to close next.
