...
7. Add incremental refresh, stale detection, and append-only event logging. File freshness detection is done for status surfaces, row-level incremental upsert is done for claim, route, route-obligation, web receipt, proof, SMT, CAS, visual, workspace-review, benchmark, notebook-run, code-run, literature, disclosure, invention, claim-chart, simulation, experiment, audit, expert-review, model-context, validation-plan, research-session, vault, and workspace-snapshot writers, and those writers now append local artifact-write events for audit ordering.
